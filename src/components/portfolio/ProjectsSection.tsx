...
function ProjectCard({ p, index, total, progress }: { p: Project; index: number; total: number; progress: any }) {
  const targetScale = 1 - (total - 1 - index) * 0.03;
  const scale = useTransform(progress, [index / total, 1], [1, targetScale]);
  return (
    <div className="h-[85vh] sticky" style={{ top: `${24 + index * 28}px` }}>
      <motion.div
        style={{ scale }}
      ...
      >
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-6">
            <div className="hero-heading font-black" style={{ fontSize: "clamp(3rem, 10vw, 140px)", lineHeight: 0.9 }}>
              {p.n}
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-xs sm:text-sm uppercase tracking-widest text-[#D7E2EA]/60">{p.category}</span>
              <span className="text-[#D7E2EA] font-medium uppercase tracking-wide text-lg sm:text-2xl md:text-3xl">{p.name}</span>
            </div>
          </div>
          <LiveProjectButton />
        </div>
        <div className="flex gap-3 sm:gap-4 md:gap-6 flex-1 min-h-0">
          <div className="flex flex-col gap-3 sm:gap-4 md:gap-6" style={{ width: "40%" }}>
            <img src={p.col1a} alt="" loading="lazy" className="w-full object-cover rounded-[40px] sm:rounded-[50px] md:rounded-[60px]" style={{ height: "clamp(130px, 16vw, 230px)" }} />
            <img src={p.col1b} alt="" loading="lazy" className="w-full object-cover rounded-[40px] sm:rounded-[50px] md:rounded-[60px]" style={{ height: "clamp(160px, 22vw, 340px)" }} />
          </div>
          <div className="flex-1">
            <img src={p.col2} alt="" loading="lazy" className="w-full h-full object-cover rounded-[40px] sm:rounded-[50px] md:rounded-[60px]" />
          </div>
        </div>
      </motion.div>
    </div>
...
export default function ProjectsSection() {
  ...
  return (
    ...
    >
      ...
      <div>
        {projects.map((p, i) => (
          <ProjectCard key={p.n} p={p} index={i} total={projects.length} progress={scrollYProgress} />
        ))}
      </div>
    </section>
  );
